...
from null_text_code import ptp_utils
...
# NUll Text Inversion Class to finetune
class NullInversion:
    ...
    def image2latent(self, image):
        with torch.no_grad():
            if (type(image) is Image.Image)| (type(image) is Image):        
    ...
    def invert(self, image_path, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        ...
        image_rec, ddim_latents = self.ddim_inversion(curr_image)

        if verbose:
            print("Null-text optimization...")
        uncond_embeddings = self.null_optimization(ddim_latents, num_inner_steps, early_stop_epsilon)
        return (curr_image, image_rec), ddim_latents[-1], uncond_embeddings
