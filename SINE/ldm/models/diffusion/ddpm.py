...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 scale_by_std=False,
                 reg_weight = 1.0,
                 scale_recon_loss=0.0,
                 recon_loss_size=64,
                 learn_pos_encoder_only=False,
                 *args, **kwargs):
        ...
        self.scale_recon_loss = scale_recon_loss
        self.recon_loss_size = recon_loss_size
        ...
        self.learn_pos_encoder_only = learn_pos_encoder_only
    ...
    def shared_step(self, batch, **kwargs):
        ...
        crop_boxes = batch.get('crop_boxes', None)
        loss = self(x, c, crop_boxes=crop_boxes)
        return loss
    ...
    def forward(self, x, c, crop_boxes=None, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
        ...
        return self.p_losses(x, c, t, crop_boxes=crop_boxes, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, crop_boxes=None, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, crop_boxes=crop_boxes)
    ...
    def p_losses(self, x_start, cond, t, noise=None, crop_boxes=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond, crop_boxes=crop_boxes)
        ...
        loss_recon = (((t / self.num_timesteps) ** 2) * F.l1_loss(F.adaptive_avg_pool2d(x_0_pred, self.recon_loss_size),
                                                                  F.adaptive_avg_pool2d(x_start, self.recon_loss_size), 
                                                                  reduction='none').mean(dim=[1, 2, 3])).mean()
        loss_dict.update({f'{prefix}/loss_recon': loss_recon})
        loss += loss_recon * self.scale_recon_loss
    ...
    def configure_optimizers(self):
        ...
        # import ipdb
        # ipdb.set_trace()
        if self.learn_pos_encoder_only:
            model_params = list()
            for name, param in self.named_parameters():
                if "pos_encode" in name:
                    model_params.append(param)
                else:
                    param.requires_grad = False
            opt = torch.optim.AdamW(model_params, lr=self.model_lr)
            return opt
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, crop_boxes=None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, crop_boxes=crop_boxes)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1, crop_boxes=crop_boxes)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc, crop_boxes=crop_boxes)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, crop_boxes=crop_boxes)
        elif self.conditioning_key == 'adm':
            cc = c_crossattn[0]
            out = self.diffusion_model(x, t, y=cc, crop_boxes=crop_boxes)
        else:
            raise NotImplementedError()
